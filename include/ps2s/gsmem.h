 ...
 */
   ...
   class CMemSlotList {
	 ...
	 CMemSlotList( int pageLength, GS::tPSM pixFormat )
	    : PageLength(pageLength), PixFormat(pixFormat) {}
	 ~CMemSlotList();
...
// needs to be after the definition of CMemSlotList
   void
   CMemSlot::RecordAccess( int curFrame ) {
 ...
 */
   ...
   class CMemManager {
	 ...
	 CMemSlot* FindLRUSlot( GS::tPSM pixFormat, int pageLength );
      ...
      public:
	 CMemManager() : LockedSlots(0, (GS::tPSM)-1), CurFrame(0) {}
	 ~CMemManager();
 ...
 */
   ...
   typedef enum { kAlignBlock, kAlignPage } tMemAlignment;
   ...
   class CMemArea {
	 ...
	 bool IsAllocated() {
	    ...
	    if ( isRes ) Slot->RecordAccess( MemManager->GetCurFrame() );
	    return isRes;
	 }
