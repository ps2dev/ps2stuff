...
namespace GS {
   ...
   void
   CDisplayEnv::SetFB( tGsrDispFB* dispFBReg,
   ...
   void
   CDisplayEnv::SetDisplay( tGsrDisplay* displayReg,
...
//        displayReg->DH = 719;
   ...
   CDisplayEnv::SendSettings( void )
   {
      ...
      reg.reg = dispfb1; reg.val = *(tU64*)&gsrDispFB1;
      ioctl( GS_fd, PS2IOC_SGSSREG, &reg );

      reg.reg = dispfb2; reg.val = *(tU64*)&gsrDispFB2;
      ioctl( GS_fd, PS2IOC_SGSSREG, &reg );
      ...
      reg.reg = bgcolor; reg.val = *(tU64*)&gsrBGColor;
      ioctl( GS_fd, PS2IOC_SGSSREG, &reg );
   }
#endif
