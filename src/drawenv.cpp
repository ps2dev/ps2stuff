 ...
 */
   ...
   void
   CDrawEnv::SetContext( GS::tContext context )
   {
   ...
   void
   CDrawEnv::InitCommon( GS::tContext context )
   {
   ...
   void
   CDrawEnv::CalculateClippedFBXYOffsets( bool addHalfPixel )
   {
   ...
   void
   CDrawEnv::SendSettings( CSCDmaPacket& packet )
   {
      bool opened_tag;
      if ( opened_tag = ! packet.HasOpenTag() )
	 packet.Cnt();
      packet.Add( &SettingsGifTag, uiNumGSRegs + 1 );
      if ( opened_tag ) packet.CloseTag();
   }

   void
   CDrawEnv::SendSettings( CVifSCDmaPacket& packet )
   {
   ...
   void
   CDrawEnv::SetFrameBufferDim( tU32 pixelW, tU32 pixelH )
   {
   ...
   void
   CDrawEnv::operator=( const CDrawEnv& otherDE )
   {
